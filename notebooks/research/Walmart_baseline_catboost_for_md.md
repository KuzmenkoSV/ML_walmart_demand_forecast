...
```python
def _create_catboost_hyperopt_space() -> dict:
    return {
        "random_state": 43,
        "iterations": 500,
        "verbose": False,
        "learning_rate": hp_loguniform("learning_rate", -6.9, -2.3),  # [log(1e-3), log(1e-1)]
        "depth": hp_choice("depth", [2, 3, 4]),
        "colsample_bylevel": hp_quniform("colsample_bylevel", 0.7, 1, 0.05),
        "l2_leaf_reg": hp_loguniform("l2_leaf_reg", -9.21, 9.21)  # [log(1e-4), log(1e+4)]
    }

def optimize_catboost(X, y, cv_folds=5, max_evals=50):
    search_space = _create_catboost_hyperopt_space()

    best_params = model_optimizer(
        model_class=CatBoostRegressor,
        model_init_params={**search_space, "cat_features": categorical_features},
        X=X,
        y=y,
        cv_folds=cv_folds,
        cv_loss=mean_squared_error,
        max_evals=max_evals,
    )
...
```python
best_catboost_params = optimize_catboost(X_train, y_train)
